...
export const SelectedPoint = createContext<SelectedPointProps>({
  sidebar: false,
  setPointId: () => {},
  toggleSidebar: () => {},
});
...
const App: FC = (): ReactElement => {
  ...
  const { setAuth, setLogin, setLogout, auth, user } =
    useContext<UserContextAttributes>(UserContext);
  ...
  useEffect(() => {
    if (typeof auth !== "undefined") {
      Adapter.getAuthProvider()
        .get()
        .then((user: UserAttributes): void => {
          setLogin!(user);
          toggleLoading(false);
        })
        .catch((e) => {
            ...
            <Switch>
              ...
              <Suspense fallback={<></>}>
                <Route
                  path="/dashboard"
                  component={
                    user?.type === "Administrator" ? Dashboard : MapDashboard
                  }
                />
              </Suspense>
            </Switch>

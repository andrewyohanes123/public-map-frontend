...
export const PointDetail: FC = (): ReactElement => {
  ...
  document.title =
    typeof point !== "undefined" ? `Dashboard - ${point.name}` : "Public Map";

  const getPoint = useCallback(() => {
    if (map) {
      PointSingle.single(parseInt(id))
        .then((resp) => {
          setPoint(resp as Point);
          // const line_string = lineString(resp.geometry.coordinates[0]);
          const boundingBox = bbox(resp.geometry);
          // @ts-ignore
          map?.fitBounds(boundingBox, { padding: 35 });
        })
        .catch((e) => {
          toast.current?.show({
            severity: "error",
  ...
  return typeof point !== "undefined" ? (
    <div
      style={{ color: "var(--surface-900)", overflow: "auto", height: "100%" }}
    >
      <Toast ref={toast} />
      ...
      <div className="p-p-3">
        <h2 style={{ color: "var(--text-color)" }} className="p-text-normal">
          {point.name}
        </h2>
        <p style={{ color: "var(--text-color-secondary)" }} className="p-mb-4">
          {point.type.name}
        </p>
            ...
            selectedPictures.length > 0 ? (
              <div
                {...getRootProps({
                  ...
                  className:
                    "p-m-3 p-p-3 p-text-center p-d-flex p-flex-column p-jc-center p-ai-center",
                })}
              >
                <input {...getInputProps()} />
                <h3 style={{ color: "var(--text-color)" }}>
                  {selectedPictures.length} gambar terpilih
                </h3>
                <p style={{ color: "var(--text-color-secondary)" }}>
                  Klik kotak ini untuk menambah gambar lokasi
                </p>
              </div>
            ) : pictures.length > 0 ? (
              <div
              ...
              >
                ...
                >
                  <i
                    ...
                    style={{ fontSize: 30, color: "var(--text-color)" }}
                  ></i>
                </div>
                <h3 style={{ color: "var(--text-color)" }}>
                  Total {pictures.length} gambar
                </h3>
                <p style={{ color: "var(--text-color-secondary)" }}>
                  Klik kotak ini untuk menambah gambar lokasi
                </p>
              ...
              >
                ...
                >
                  <i
                    className="pi pi-fw pi-image"
                    style={{ fontSize: 50, color: "var(--text-color)" }}
                  ></i>
                </div>
      ...
      <div className="p-p-3">
        {pictures.map((pic) => (
          <ImagePreviewCard
            key={pic.id}
            src={`${REACT_APP_IP_ADDRESS}:${REACT_APP_PORT}/picture/${pic.id}`}
            onRemove={() => deletePicture(pic)}
          />
        ))}
      </div>

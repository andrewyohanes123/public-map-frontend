    ...
    <div className="sidebar-container p-mt-2">
      {loading || typeof Point === "undefined" ? (
        ...
        <div className="p-p-3">
          ...
          <p className="p-mb-1">
            Luas daerah&nbsp;:&nbsp;{point?.surface_area} Ha
          </p>
          <p>
          ...
          {false && (
            <>
              {point?.pictures?.length === 0 ? (
                ...
                <>
                  <h4 className="p-mt-2 p-mb-2">
                    {point!.pictures!.length > 1 ? "Foto - Foto" : "Foto"}
                  </h4>
                  {point?.pictures?.map((picture) => (
                    <SidebarCoverImage
                      key={picture.id}
                      src={`${REACT_APP_IP_ADDRESS}:${REACT_APP_PORT}/picture/${picture.id}`}
                    />
                  ))}
                </>
              )}
